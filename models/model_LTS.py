...
class Network_LTS(nn.Module):
    def __init__(self, input_channels=6):
        super(Network_LTS, self).__init__()
        ...
        self.conv_last = nn.Conv2d(in_channels=64, out_channels=4, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, parse_limb_t, limb_input, skeleton, densepose):
        input = torch.cat((parse_limb_t, limb_input, skeleton, densepose), axis=1)    # [b, 1+3+1+1, h, w]
        ...
        d0 = self.decode1(d1)       
        limb = self.conv_last(d0)    # [b,4,h,w]
        limb_r, limb_w = torch.split(limb, [3, 1], dim=1)
        limb_r = self.sigmoid(limb_r) * parse_limb_t
        limb_w = self.sigmoid(limb_w) * parse_limb_t
        return limb_r, limb_w

if __name__ == '__main__':
    parse_limb_t = torch.randn(4, 1, 512, 384).cuda()
    limb_input = torch.randn(4, 3, 512, 384).cuda()
    skeleton = torch.randn(4, 1, 512, 384).cuda()
    densepose = torch.randn(4, 1, 512, 384).cuda()

    model = Network_LTS().cuda()
    limb_r, limb_w = model(parse_limb_t, limb_input, skeleton, densepose)
    print("limb_r:", limb_r.shape)
    print("limb_w:", limb_w.shape)  
